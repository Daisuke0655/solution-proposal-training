...
// PDF Workerのパス設定 (重要!)
// node_modules/pdfjs-dist/build/pdf.worker.min.js をコピーして
// Next.js プロジェクトの public フォルダ内に pdf-worker.js という名前で配置してください。
pdfjsLib.GlobalWorkerOptions.workerSrc = '/pdf-worker.js';
// あるいは、CDNから読み込む場合 (pdfjsLib.version はインストールされているバージョンに合わせてください)
// pdfjsLib.GlobalWorkerOptions.workerSrc = `//cdnjs.cloudflare.com/ajax/libs/pdf.js/${pdfjsLib.version}/pdf.worker.min.js`;


// ****** 警告：APIキーをフロントエンドに直接記述するのは危険です！ ******
// 本番環境では必ずバックエンド経由で呼び出すか、環境変数を使用してください。
const GEMINI_API_KEY = "AIzaSyDmM81qMtcukHF-NIp6kvvuzKU6p30F99Y";

const ai = new GoogleGenerativeAI(GEMINI_API_KEY);
const model = ai.getGenerativeModel({ model: 'gemini-pro' }); // 使用するモデルを指定
...
const SearchPage = () => {
  ...
  const [pdfText, setPdfText] = useState(''); // PDFから抽出されたテキスト
  const [geminiResponse, setGeminiResponse] = useState(''); // Gemini APIの応答
  const [geminiError, setGeminiError] = useState(null); // Gemini APIのエラー

  // プログラム起動時にpublicフォルダのPDFを読み込むuseEffect
  useEffect(() => {
    let isMountedForPdf = true; // PDF処理用のマウントフラグ

    const fetchAndExtractPdf = async () => {
      setPdfText('');
      setGeminiError(null); // PDF読み込みエラーもここでリセット

      const pdfUrl = '/kekka.pdf'; // publicフォルダにあるPDFファイルのパス

      try {
        const response = await fetch(pdfUrl);
        if (!response.ok) {
          throw new Error(`HTTP error! status: ${response.status} from ${pdfUrl}`);
        }
        ...
        if (isMountedForPdf) {
          setPdfText(fullText); // 抽出したテキストをStateに保存
          console.log("PDFからテキストを抽出しました。文字数:", fullText.length);
        }
      } catch (e) {
        if (isMountedForPdf) {
          console.error('PDF読み込み/解析エラー:', e);
          setGeminiError(`PDFファイルの読み込みまたは解析に失敗しました: ${e.message}`);
        }
      }
    };
    ...
    // クリーンアップ関数
    return () => {
      isMountedForPdf = false;
    };
  }, []); // コンポーネントマウント時に一度だけ実行 (依存配列が空)

  // Gemini API呼び出し用の関数 (useCallback でメモ化)
  const callGeminiApi = useCallback(async (currentSearchTerm, pdfContent) => {
    // 検索ワードもPDFコンテンツも空の場合はAIに送信しない
    if (!currentSearchTerm.trim() && !pdfContent.trim()) {
      setGeminiError('検索ワードまたはPDFコンテンツが空のため、AIに送信できません。');
      return;
    }

    setIsLoading(true); // AI呼び出し開始時にローディング
    setGeminiError(null);
    setGeminiResponse('');

    try {
      let promptForGemini = `以下の情報に基づいて専門家を分析し、どのような課題解決に貢献できるか提案してください。\n\n`;

      if (currentSearchTerm.trim()) {
        promptForGemini += `検索ワード: "${currentSearchTerm}"\n`;
        // 検索結果のユーザー情報もプロンプトに含める
        if (searchResults.length > 0) {
            promptForGemini += `関連するユーザー情報:\n`;
            promptForGemini += searchResults.map(user =>
                `- 名前: ${user.name}, 部署: ${user.department}, スキル: ${user.skills.join(', ')}, 最近の活動: ${user.recentActivity}`
            ).join('\n') + '\n';
        }
      }

      // PDFテキストが読み込まれていればプロンプトに含める
      if (pdfContent.trim()) {
        promptForGemini += `\n以下のPDFから抽出されたテキストも考慮に入れてください:\n\n${pdfContent}\n`;
      }
      ...
      setGeminiError(`Gemini APIからの情報取得に失敗しました: ${e.message}`);
    } finally {
      setIsLoading(false); // AI呼び出し完了時にローディング終了
    }
  }, [searchResults, pdfText]); // searchResults と pdfText も依存配列に含める
  ...
  const handleSearch = () => {
    if (!searchTerm.trim()) {
      setSearchResults([]);
      setHasSearched(true);
      // setGeminiResponse(''); // 検索ワードが空の時はAI応答もクリア
      // setGeminiError(null);
      // setIsLoading(false);
      return;
    }
    // setIsLoading(true); // ここではなく、useEffect内で管理
    setHasSearched(true); // 検索が実行されたことを示す
    ...
    setSearchResults(results); // searchResultsが更新され、次のuseEffectが発火
  };

  // searchTerm または pdfText の変更を監視し、APIを呼び出すuseEffect (デバウンス処理)
  // PDF読み込み完了後や検索ワード入力後にGeminiを呼び出す
  useEffect(() => {
    let isMountedForApi = true;
    let debounceTimer;

    // 検索ワードもPDFコンテンツも準備できていれば（両方空でなければ）APIを呼び出す
    if (searchTerm.trim() !== '' || pdfText.trim() !== '') {
        setIsLoading(true); // 検索/AI処理開始時にローディング開始
        setError(null); // 前回のUIエラーをクリア
        setGeminiResponse(''); // 前回のAI応答をクリア
        setGeminiError(null); // 前回のAIエラーもクリア

        debounceTimer = setTimeout(() => {
            if (isMountedForApi) {
                callGeminiApi(searchTerm, pdfText); // API呼び出しを実行
            }
        }, 500); // 500ミリ秒のデバウンス
    } else {
        // 検索ワードもPDFコンテンツも空の場合、状態をリセット
        clearTimeout(debounceTimer);
        // setSearchResults([]); // ユーザー検索結果は残したい場合もあるので注意
        setGeminiResponse('');
        setGeminiError(null);
        setIsLoading(false);
        // setHasSearched(false); // この状態を保持するかはUIによる
    }

    // クリーンアップ関数
    return () => {
      clearTimeout(debounceTimer);
      isMountedForApi = false; // 古いAPIコールの結果でStateが更新されないようにする
    };
  }, [searchTerm, pdfText, callGeminiApi]); // searchTerm, pdfText, callGeminiApi が変更されるたびに useEffect が再実行される
  ...
  return (
    <div className="max-w-4xl mx-auto">
      ...
      {geminiError && ( // Gemini APIまたはPDF読み込みのエラー表示
      ...
      {/* Geminiからの応答を表示するセクション */}
      {!isLoading && geminiResponse && (
        <div className="mt-6 p-4 border rounded-lg bg-blue-50">
          <h2 className="text-xl font-semibold mb-3">AIによる分析結果:</h2>
          <pre className="whitespace-pre-wrap text-gray-800 bg-white p-3 rounded">{geminiResponse}</pre>
        </div>
      )}

      {/* 初期状態のメッセージ (検索前かつエラーやローディング中でない場合) */}
      {!isLoading && !hasSearched && !geminiResponse && !geminiError && (
        <div className="text-center py-10 bg-white shadow-lg rounded-xl p-8">
